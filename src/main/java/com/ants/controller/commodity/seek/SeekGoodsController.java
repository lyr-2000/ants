package com.ants.controller.commodity.seek;
 ...
 */
@Controller
@RequestMapping(value = "/ants/seekGoods")
public class SeekGoodsController {

    @Autowired
    private SeekGoodsService seekGoodsService;
    ...
    public Map mySeekGoods(HttpServletRequest request) {
        ...
        List<SeekGoods> seekList = seekGoodsService.listMySeekGoods(parameterMap);
    ...
    public Map<String, String> releaseSeek(//@RequestParam MultipartFile photo,
        ...
        //将商品信息添加到数据库中
        int result = seekGoodsService.insertSeekGoods(seek);
        if (result > 0) {
