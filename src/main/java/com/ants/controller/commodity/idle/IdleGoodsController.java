package com.ants.controller.commodity.idle;
 ...
 */
@Controller
@RequestMapping(value = "/ants/idleGoods")
...
//        uploadIdle = Upload.uploadPhoto(photo, request);
    ...
    public Map getClassifyParentAndChild(
        ...
        switch (classifyStatus) {
            case 1:
                //将父类的分类情况发送到前端界面
                List<ParentClass> parentClasses = classifyService.listParentClassificationHasOthers();
            ...
            case 2:
                ...
                //根据父类id获取对应的子类分类情况
                List<ChildClass> childClass = classifyService.listChildClassification(parentClass);
    ...
    public Map myIdleGoods(HttpServletRequest request) {
        ...
        //获取此账号下闲置的所有物品信息
        List<Goods> idleList = idleGoodsService.listMyIdleGoods(parameterMap);
